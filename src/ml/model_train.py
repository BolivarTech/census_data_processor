...
def main(args):
    ...
    # Else Train and save a model.
        ...
        # save model  to disk
        with open(os.path.join(savepath, filename[0]), 'wb') as file:
            pickle.dump(model, file)
        with open(os.path.join(savepath, filename[1]), 'wb') as file:
            pickle.dump(encoder, file)
        with open(os.path.join(savepath, filename[2]), 'wb') as file:
            pickle.dump(lb, file)
    ...
    # iterate through the categorical features and save results to log and txt
    # file
